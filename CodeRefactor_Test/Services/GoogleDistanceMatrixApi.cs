...
namespace CodeRefactor_Test.Services
{
    public class GoogleDistanceMatrixApi
    {
        ...
        private string pscAddress;
        private string cscAddress;

        public GoogleDistanceMatrixApi(GeoCoordinate origin, ClosestHospitals closestHospitals)
        {
            SetupDefaultKeys();
            SetupApi(new List<string> { origin.ToString() }, closestHospitals);
        }
        ...
        private void SetupApi(IEnumerable<string> originAddresses, ClosestHospitals closestHospitals)
        {
            this.originAddresses = originAddresses;
            this.pscAddress = closestHospitals.PscName;
            this.cscAddress = closestHospitals.CscName;
        }
        ...
        public async Task<ResponseDistanceMatrix> GetResponseNew()
        {
            using (var client = new HttpClient())
            {
                //GetRequestUrlNew();
                var uriList = GetRequestUrlNew();
                var pscUri = uriList[0];
                var cscUri = uriList[1];
                var uri = new Uri(pscUri);
        ...
        {
            var originsUrl = string.Join("|", originAddresses.Select(WebUtility.UrlEncode));
            var destinationPscUrl = pscAddress;
            var destinationCscUrl = cscAddress;
            var departureTimeUrl = ((47 * 365 * 24 * 60 * 60) + (11 * 24 * 60 * 60) + (181 * 24 * 60 * 60) + (5 * 60 * 60) + (8 * 7 * 24 * 60 * 60)).ToString();
            string pscUri = $"{apiUrl}?departure_time=now&traffic_model=optimistic&origins={originsUrl}&destinations={destinationPscUrl}&key={apiKey}";
            string cscUri= $"{apiUrl}?departure_time=now&traffic_model=optimistic&origins={originsUrl}&destinations={destinationCscUrl}&key={apiKey}";
